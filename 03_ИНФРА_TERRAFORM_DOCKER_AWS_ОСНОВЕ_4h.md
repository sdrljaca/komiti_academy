...
### 4.4) State као concept

Terraform state је кључни артефакт, али није исто што и један конкретан фајл.

У најједноставнијем случају state је локално сачуван у фајлу `terraform.tfstate`, али у озбиљнијим setup-има може живјети у remote backend-у. Значи:
- `state` = снимљено Terraform знање о стварном стању ресурса,
- `terraform.tfstate` = један чест, локални носач тог state-а.
...
#### 4.7.2) `terraform.tfstate` и `terraform.tfstate.backup`

Ово је мјесто гдје concept из `### 4.4) State као concept` у root stack-у добија свој најчешћи локални облик: фајл `terraform.tfstate`.

Практично:
- `terraform.tfstate` је локални snapshot Terraform state-а за тај stack,
- `terraform.tfstate.backup` је backup претходног локалног snapshot-а,
- оба су runtime артефакти, не source code.

Важно је да кандидат овдје јасно раздвоји 2 нивоа:
- `state` је concept који може живјети и локално и remote,
- `terraform.tfstate` је конкретан локални фајл који тај concept носи у simple/local backend моделу.

Зато ове фајлове не читаш као business/config source, него као machine-managed runtime truth коју Terraform одржава.
