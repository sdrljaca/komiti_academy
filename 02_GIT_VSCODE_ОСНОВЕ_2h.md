...
Важно: Git **не прави посебан директоријум на диску за сваку грану**. Имаш један једини фолдер (нпр. `C:\dev\KomITi\komiti_academy`), и Git мијења садржај тог истог фолдера кад прелазиш са гране на грану. Кад урадиш `git checkout main`, Git у тај фолдер замијени фајлове стањем гране `main` из локалне `.git/` базе. Кад урадиш `git checkout 2026-03-12-features`, Git у исти тај фолдер замијени фајлове стањем те feature гране. Дакле:
- `checkout` не download-ује ништа са GitHub-а — само преписује фајлове на диску из локалне `.git/` базе гдје Git чува сва стања свих грана, компримовано,
- фајлове који се разликују Git **препише** (overwrite), фајлове који постоје само на старој грани **обрише**, а фајлове који постоје само на новој грани **креира**,
...
├── refs/         ← ту су "показивачи" (гране, тагови) — текст фајлови
├── HEAD          ← текст фајл који каже на којој си грани
├── config        ← подешавања репоа (remote URL итд.)
├── index         ← staging area
└── ...
```
- `objects/` = сваки фајл, сваки commit и свака директоријумска структура коју си икад commit-овао постоји овдје као компримовани blob објекат; Git их адресира преко SHA-1 hash-а (онај дуги низ слова и бројева),
- `refs/` = гране и тагови; нпр. фајл `refs/heads/main` садржи само један ред — hash commit-а на који `main` тренутно показује,
- `HEAD` = текст фајл који каже на којој си грани; отвори га и видјећеш нешто као `ref: refs/heads/main`,
- `index` = staging area; ту Git памти шта си staged-овао за наредни commit.

Практична посљедица: кад урадиш `git checkout main`, Git погледа `refs/heads/main`, нађе hash задњег commit-а, из `objects/` извуче компримоване верзије фајлова и препише их у твој радни фолдер. Зато `checkout` не download-ује ништа са GitHub-а — све је већ ту, на твом диску, у `.git/` фолдеру.

Ако желиш да видиш садржај фајла са друге гране без да мијењаш свој радни фолдер, постоји команда `git show <грана>:<путања>`, нпр. `git show main:README.md` — Git ће из `.git/objects/` извући ту верзију фајла и приказати је у терминалу, без да ти нешто препише на диску.
