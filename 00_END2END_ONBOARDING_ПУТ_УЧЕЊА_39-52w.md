...
Задаци у пројекту
Док читаш `komiti_academy` learning библиотеку, добијаћеш пројектне задатке који се налазе на крају сваког документа (у поглављима `99) Задатак на komiti_academy пројекту за кандидата` и `99) Рјешења`).
