...
- Кроз Explorer отвараш фајл једним кликом, а фолдер шириш и скупљаш стрелицама поред назива.
...
- Ако у VS Code-у урадиш `Open Folder`, отворићеш један фолдер као свој радни workspace.
- Ако у већ отворен workspace додаш још један фолдер, добијаш `multi-root workspace`, тј. workspace са више root директоријума.
- Практично: `Open Folder` је најједноставнија варијанта кад радиш само у једном репоу. `Add Folder to Workspace` има смисла кад мораш истовремено гледати више директоријума, нпр. `odoo4komiti` и издвојени `odoo4komiti4academy`.

### Шта се мијења кад отвориш workspace са више директоријума
- Explorer ће приказати више top-level root фолдера.
- Search, Explorer и Git контекст више нису само за један фолдер, него за све фолдере у workspace-у.
- Мораш више пазити у ком root-у правиш фајл или покрећеш команду.

### Да ли можеш имати више workspace-ова истовремено
- Да, можеш.
- Најчешћи случај је да отвориш више одвојених VS Code прозора, и сваки прозор има свој workspace.
- То значи да можеш, на примјер, у једном прозору имати `odoo4komiti`, а у другом `odoo4komiti4academy`.
- Можеш и у једном прозору направити `multi-root workspace` и убацити оба фолдера у исти workspace, али то није исто као два одвојена workspace-а.
...
### Какав је однос агента и workspace-а
- Агент као што сам ја не ради у апстрактном вакууму, него у оквиру workspace контекста који је тренутно отворен у VS Code-у.
- То практично значи да агент види и претражује фолдере који су дио тог workspace-а.
- Ако су у workspace додати `odoo4komiti` и `odoo4komiti4etn`, агент може радити у оба јер су оба у истом VS Code контексту.
- Ако неки фолдер није додат у тренутни workspace, агент га не треба претпостављати као доступан и зато је битно увијек знати шта је стварно отворено.
- Зато `workspace` није само визуелни појам у VS Code-у, него граница контекста за претрагу, читање фајлова, терминал и рад агента.

### Како се фајл или директоријум отварају у пракси
- Кроз Explorer кликнеш на директоријум да га прошириш.
- Кликнеш на фајл да га отвориш у editor-у.
- Десни клик на директоријум ти даје опције као што су `New File` и `New Folder`.
- Ако не знаш гдје је фајл, не лутaj само кроз Explorer него комбинуј Explorer са претрагом.
...
## 2) Git није исто што и GitHub
- `Git` је систем за version control.
- `GitHub` је платформа на којој живи remote репо.
...
## 5.1) Шта значи доњи лијеви угао у VS Code-у
У доњем лијевом углу VS Code-а често видиш кратак Git status за активни repo.
На примјеру типа:
- `odoo4komiti`
- `2026-03-11-features*`
- иконицу кружних стрелица
то најчешће значи:
...
- кружне стрелице = Git sync/refresh зона; ту VS Code показује или нуди fetch/pull/push/sync, а ако се иконица врти, обично је Git операција у току.
...
- у ком repo-у си,
- на којој branch-и си,
- да ли имаш локалне измјене.
За агента је ово користан сигнал, али не и магична гаранција.
- Git indicator говори агенту који је тренутни branch контекст. У истом clone-у и истом working tree-ју агент не може радити на другој локалној грани од оне коју indicator показује. Ако indicator показује `2026-03-11-features`, онда агент у том контексту ради над `2026-03-11-features`.
